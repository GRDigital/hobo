...
impl<'a, 'b> From<roxmltree::Node<'a, 'b>> for crate::BasicElement<web_sys::SvgElement> {
	fn from(node: roxmltree::Node) -> Self {
		let element = web_sys::SvgElement::from(wasm_bindgen::JsValue::from(
			crate::dom().create_element_ns(Some("http://www.w3.org/2000/svg"), node.tag_name().name()).expect("can't create svg element"),
		));
