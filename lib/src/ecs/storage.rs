...
impl<'a, Component, Inner> std::ops::DerefMut for StorageGuard<'a, Component, Inner> where
	...
	Inner: std::ops::DerefMut<Target = SimpleStorage<Component>>,
{
	fn deref_mut(&mut self) -> &mut Self::Target { &mut *self.1.as_mut().unwrap() }
}
	...
	Inner: std::ops::DerefMut<Target = SimpleStorage<Component>>,
{
	fn drop(&mut self) {
		...
		let set = {
			...
			storage.added.iter().chain(storage.modified.iter()).chain(storage.removed.iter()).copied().collect::<HashSet<_>>()
		};
