...
fn main() {
	assert!(cmd!(cargo release --no-confirm).current_dir("derive").status().unwrap().success());
	...
	sleep(Duration::from_secs(60));
	assert!(cmd!(cargo publish).current_dir("css/css_macros_decl").status().unwrap().success());
	sleep(Duration::from_secs(60));
